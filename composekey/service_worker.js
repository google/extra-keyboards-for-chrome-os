...
// TODO(nkillewald): These are the constants from KeyboardEvent, but we don't
// seem to have access to that class from within a ServiceWorker. Either figure
// out how to get those in so these consts aren't necessary or confirm this is
// the best way to handle this and remove this TODO.
const KBE_DOM_KEY_LOCATION_STANDARD = 0;
const KBE_DOM_KEY_LOCATION_LEFT = 1;
const KBE_DOM_KEY_LOCATION_RIGHT = 2;
const KBE_DOM_KEY_LOCATION_NUMPAD = 3;

/** @type {Object<string, string>!} */
const builtinSequences = Object.freeze({
...
Object.entries(specialSymToKey).forEach(([sym,
                                          key]) => specialKeyToSym[key] = sym);
...
function symToEvent(sym) {
  let location = KBE_DOM_KEY_LOCATION_STANDARD;
  ...
  if (key.startsWith('KP_')) {
    location = KBE_DOM_KEY_LOCATION_NUMPAD;
    key = sym.substring(3);
  } else if (sym.endsWith('_L')) {
    location = KBE_DOM_KEY_LOCATION_LEFT;
    key = sym.substring(0, sym.length - 2);
  } else if (sym.endsWith('_R')) {
    location = KBE_DOM_KEY_LOCATION_RIGHT;
    key = sym.substring(0, sym.length - 2);
...
function eventToSyms(keyData) {
  ...
  if (location === undefined) {
    ...
    switch (key) {
      ...
      case 'Alt':
      case 'Meta':
        location = keyData.code.endsWith('Right') ? KBE_DOM_KEY_LOCATION_RIGHT :
                                                    KBE_DOM_KEY_LOCATION_LEFT;
        break;
      ...
      case 'PageUp':
        location = keyData.code.startsWith('Numpad') ?
            KBE_DOM_KEY_LOCATION_NUMPAD :
            KBE_DOM_KEY_LOCATION_STANDARD;
        break;

      default:
        location = KBE_DOM_KEY_LOCATION_STANDARD;
    }
  }

  if (key == 'Enter' && location == KBE_DOM_KEY_LOCATION_NUMPAD) {
  ...
  switch (location) {
    case KBE_DOM_KEY_LOCATION_STANDARD:
      return [sym];
    case KBE_DOM_KEY_LOCATION_RIGHT:
      return [sym + '_R'];
    case KBE_DOM_KEY_LOCATION_LEFT:
      return [sym + '_L'];
    case KBE_DOM_KEY_LOCATION_NUMPAD:
      return ['KP_' + sym];
    default:
...
class Modifiers {
   ...
   */
  equals(other) {
    return this.control === other.control && this.shift === other.shift &&
...
/**
 * Rebuilds a proper Modifiers class instance from a potential plain
 * object retrieved from storage.
 * @param {Object!} storedObj
 * @return {Modifiers!}
 */
function rebuildModifiersFromStorage(storedObj) {
  if (storedObj instanceof Modifiers) {
    return storedObj;
  } else {
    return Object.assign(new Modifiers(), storedObj);
  }
}
...
class Result {
  ...
  /**
   * Sends keydata to the input IME.
   * @param {{extensionId: string, keyCode: number, type: string}} keyData
   */
  send(keyData) {
    ...
    chrome.input.ime.sendKeyEvents(
        ...
        () => {
          if (chrome.runtime.lastError) {
            console.error(
                'Error sending key events:', chrome.runtime.lastError);
          }
        });
  }
}
...
/**
 * Rebuilds a proper Result class instance from a potential plain object
 * retrieved from storage.
 * @param {Object!} storedObj
 * @return {Result!}
 */
function rebuildResultFromStorage(storedObj) {
  if (storedObj instanceof Result) {
    return storedObj;
  } else {
    return new Result(storedObj.string, storedObj.keysym);
  }
}
...
/**
 * Rebuilds a proper EventState class instance from a potential plain
 * object retrieved from storage. Storage only allows for serialized
 * JSON, you see.
 * @param {Object!} storedObj
 * @return {EventState!}
 */
function rebuildEventStateFromStorage(storedObj) {
  if (storedObj instanceof EventState) {
    return storedObj;
  } else {
    // Make sure the mods and result fields are classes as well (if need
    // be).
    if (!(storedObj.mods instanceof Modifiers)) {
      storedObj.mods = rebuildModifiersFromStorage(storedObj.mods);
    }

    if (storedObj.result !== null && !(storedObj.result instanceof Result)) {
      storedObj.result = rebuildResultFromStorage(storedObj.result);
    }

    // Also, the nextKeys object. That one requires recursion.
    for (const [key, states] of Object.entries(storedObj.nextKeys)) {
      for (const state of states) {
        rebuildEventStateFromStorage(state);
      }
    }

    return Object.assign(
        new EventState(storedObj.mods, storedObj.builtin), storedObj);
  }
}
  ...
  for (let suffixStates of Object.values(finalState.nextKeys)) {
    ...
    for (let state of suffixStates) {
      if (state !== finalState && !state.builtin) {
        console.warn(
            'New sequence is a prefix of existing sequence(s):', state);
...
function parseEvent(line) {
  ...
  if ((match = /^\s*None\s*/.exec(rest))) {
    ...
    while (
        (match = /^\s*([~]\s*)?(Ctrl|Lock|Caps|Shift|Alt|Meta)\b/.exec(rest))) {
      rest = rest.substring(match[0].length);
...
/**
 * Sets the compose key and keepModifier settings.
 * @param {{key: string, keepModifier: boolean}} settings
 */
function setKey(settings) {
  chrome.storage.sync.set(
      {key: settings.key, keepModifier: settings.keepModifier});
...
let onComposeKeyLoaded = null;

const defaultComposeFile = 'include "%L"\n';

/**
 * The current raw compose file in XCompose format.
 * @return {Promise<string>!}
 */
async function getComposeFile() {
  // The contents of the compose file are either stored in sync or local, with
  // sync taking precedence. If it's not in either, we use a default compose
  // file.
  return (await chrome.storage.sync.get('composeFile')).composeFile ??
      await chrome.storage.local.get({'composeFile': defaultComposeFile})
          .composeFile;
}

/**
 * Sets the contents of the compose file (to local or sync storage) and parses
 * it into the root state (to session storage).
...
function setComposeFile(content) {
  ...
  // The raw contents of the compose file get stored away, to be parsed into an
  // EventState tree on load.
  storeComposeFile(content);

  const parsed = parseComposeFile(content);

  // The parsed EventState tree is just cached in session storage.
  chrome.storage.session.set({'rootState': parsed.root});
  composeFile = content;
  fileStatus.warnings = parsed.warnings;
  fileStatus.errors = parsed.errors;
  resetState();
}

/**
 * Stores the contents of the compose file to either local or sync storage,
 * depending on whether or not it fits in sync storage. If it also doesn't fit
 * in local storage, then we've got problems.
 * @param {string} content
 */
function storeComposeFile(content) {
  chrome.storage.sync.set({composeFile: content}, () => {
    if (!chrome.runtime.lastError) {
      console.log(
          'Stored compose file to Chrome sync (%d characters).',
    ...
    console.warn(
        'Failed to store compose file to Chrome sync:',
        chrome.runtime.lastError);
    console.warn('Falling back to local storage and clearing sync storage.');
    chrome.storage.local.set({'composeFile': content});
    console.log(
        'Stored compose file to local storage (%d characters).',
        lengthInCodepoints(content));
    composeFile = content;
    ...
    chrome.storage.sync.remove('composeFile', () => {
      if (chrome.runtime.lastError) {
        console.warn(
            'Failed to remove stored compose file from Chrome sync:',
            chrome.runtime.lastError);
      }
    });
  });
...
/**
 * Clears the compose file and resets the state. This clears both sync and local
 * storage, then resets the contents to the default compose file via
 * setComposeFile, with the implications therein regarding whether it goes to
 * sync or local storage.
 *
 * As a side effect of reloading the default compose file, this will also
 * ultimately repopulate the sync and/or local storage with the default compose
 * file, and re-parse that into the root state.
 */
...
/** @type {function(string)} */
let onComposeFileLoaded = null;

/**
 * Loads a compose file into local storage.
 */
async function loadLocalComposeFile() {
  const composeFile = await chrome.storage.sync.get('composeFile');
  let content = composeFile.composeFile;
  if (content == null) {
    setComposeFile(defaultComposeFile);
  } else {
    console.log(
        'Loaded compose file from local storage (%d characters).',
        lengthInCodepoints(content));
    setComposeFile(content);
  }

  if (onComposeFileLoaded) {
    onComposeFileLoaded(await getComposeFile());
  }
}

chrome.storage.sync.get('composeFile', async (stored) => {
  if (chrome.runtime.lastError) {
    console.warn(
        'Failed to load compose file from Chrome sync:',
        chrome.runtime.lastError);
  ...
  console.log(
      'Loaded compose file from Chrome sync (%d characters).',
 ...
 * @param {Array<EventState!>!} value
 */
function setStates(value) {
  chrome.storage.session.set({'states': value});
  states = value;
}
...
function setComposing(value) {
  chrome.storage.session.set({'composing': value});
  composing = value;
}
 ...
 */
function setKeyDownSuppressed(value) {
  chrome.storage.session.set({'keyDownSuppressed': [...value]});
  keyDownSuppressed = value;
}
 ...
 */
function setComposeOnKeyUp(value) {
  chrome.storage.session.set({'composeOnKeyUp': value});
  composeOnKeyUp = value;
}
...
function resetState() {
  setStates([rootState]);
  setComposing(false);
  setKeyDownSuppressed(new Set());
  setComposeOnKeyUp(false);
}
 ...
 * @return {Promise<Array<EventState!>!>!}
 */
let states = [];

// Initialize all values from storage each time the worker starts up.
// Variables aren't preserved otherwise when dealing with a
// ServiceWorker, you see, and a ServiceWorker can go away at just about
// any time.
const initPromise =
    chrome.storage.sync
        .get(
            {'key': 'AltRight', 'composeFile': null, 'keepModifier': true})
        .then((options) => {
          composeKey = options.key;
          keepModifier = options.keepModifier;

          if (options.composeFile === null) {
        ...
        .then((options) => {
          ...
          return chrome.storage.session.get({
            'keyDownSuppressed': [],
            'rootState': new EventState(modifiersAny),
            'states': undefined,
            'composing': false,
        ...
        .then((options) => {
          keyDownSuppressed = new Set(options.keyDownSuppressed);
          composing = options.composing;
          composeOnKeyUp = options.composeOnKeyUp;

          rootState = rebuildEventStateFromStorage(options.rootState);
          if (options.states === undefined) {
            states = [rootState];
          } else {
            const fetchedStates = options.states;
            states = [];

            for (const state of fetchedStates) {
              states.push(rebuildEventStateFromStorage(state));
            }
          }
        });
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  switch (message.command) {
    case 'getOptions': {
      sendResponse({composeKey, composeFile, keepModifier});
      return true;
    }
    case 'getFileStatus': {
      sendResponse(fileStatus);
      return true;
    }
    case 'storeKeyOptions': {
      if (!message.data) {
        sendResponse({error: 'No data sent with storeKeyOptions'});
        return true;
      }
    ...
    case 'storeComposeFile': {
      if (!message.data) {
        sendResponse({error: 'No data sent with storeComposeFile'});
        return true;
      }

      storeComposeFile(message.data);
      sendResponse({result: 'OK'});
      return true;
    }
    case 'clearComposeFile': {
      clearComposeFile();
      sendResponse({result: 'OK'});
      return true;
    }
...
chrome.input.ime.onKeyEvent.addListener((engineID, keyData, requestId) => {
  // WAIT! First, make sure we've got all the settings in place before we do any
  // processing.
  initPromise.then(() => {
    const suppress = true;
    ...
    // keyup so that it still works as a modifier. (Pressing some other key
    // while holding the compose key cancels the compose-on-key-up action.)
    //
    // Otherwise, fire the compose event on keydown so that we correctly
    // suppress its regular action.
    //
    // On ChromeOS, the Meta key by itself is an accelerator, so don't treat it
    // as a transient modifier.
    ...
    if (keyData.code == composeKey) {
      if (keyData.type == 'keyup') {
        ...
        if (isModifier && keepModifier) {
          ...
          // other keystrokes.
          chrome.input.ime.keyEventHandled(requestId, propagate);
          return;
        }
        isComposeEvent = true;
        isModifier = false;
    ...
    if (keyData.type == 'keyup' && !isComposeEvent) {
      if (keyDownSuppressed.has(key)) {
        ...
        chrome.input.ime.keyEventHandled(requestId, suppress);
        return;
      }
      chrome.input.ime.keyEventHandled(requestId, propagate);
      return;
    ...
    for (const sym of syms) {
      ...
      if (nextStates.length == 0) {
        // The key does not extend any candidate sequence. If we haven't
        // consumed any keystrokes yet, or the key is a modifier for the next
        // keystroke, propagate it; otherwise, suppress it as normal.
        if (!composing || isModifier) {
          chrome.input.ime.keyEventHandled(requestId, propagate);
          return;
        }
